...
*************************************************************************/
static void i2c_start_wait(unsigned char address)
{
    uint8_t   twst;
...
*************************************************************************/
static unsigned char i2c_rep_start(unsigned char address)
{
...
{
	if(i2c_start(i2c->address+I2C_WRITE) != 0)
	{
		i2c_stop();
		return 1;
	}
...
{
	uint8_t retval = 0;
	i2c_start_wait(i2c->address+I2C_READ);
	retval =  i2c_readNak();
	i2c_stop();
	return retval;
}
...
uint8_t TWIM_receive_msg_cmd(i2c_device_t *i2c, uint8_t cmd, uint8_t *data, uint8_t count)
{
	...
	/* OK everything seems to be just fine send the cmd to the slave */
	i2c_write(cmd);

	/* Send a repeated start condition to change the direction */
	i2c_rep_start(i2c->address+I2C_READ);

	/* OK, now receive the bytes from the slave */
	for (i=0;i<(count-1);i++)
	{
		data[i] =  i2c_readAck();
	}
